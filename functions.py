...
def iterator(list_of_files):
    speeds = []
    for i, a in enumerate(list_of_files):
        if i < len(list_of_files)-1:
            im1 = f"images/{a}"
            im2 = f"images/{list_of_files[i+1]}"
            v = calculate_speed(im1, im2)
            speeds.append([a, list_of_files[i+1], v])
        else:
            break
